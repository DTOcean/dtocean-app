...
def test_FileManagerWidget_set_path_load(qtbot, mocker, fm_widget_both):
    ...
    file_name = "mock.in"
    mocker.patch('dtocean_app.widgets.central.QtGui.QFileDialog.exec_',
                 autospec=True)
    mocker.patch('dtocean_app.widgets.central.QtGui.QFileDialog.selectedFiles',
    ...
    assert fm_widget_both.buttonBox.button(
                                        QtGui.QDialogButtonBox.Ok).isEnabled()
...
def test_PlotManagerWidget_plot_list(mocker, pm_widget):
    ...
    mocker.patch('dtocean_app.widgets.central.get_current_filetypes',
