...
def test_SimpleData_input_valid_values(mocker, qtbot, structure):
    ...
    assert widget.call_args.args == (structure.parent,
                                     meta.valid_values)
    assert widget.call_args.kwargs == {'unit': meta.units[0],
                                       'experimental': None}
...
@pytest.mark.parametrize("ttype, input_widget", [
                                ("float", "FloatSelect"),
                                ("int", "IntSelect")])
def test_SimpleData_input_number_closed(mocker,
                                        qtbot,
                                        structure,
                                        ttype,
                                        input_widget):
    ...
    assert widget.call_args.args[:2] == (structure.parent,
                                         meta.units[0])
...
@pytest.mark.parametrize("ttype, input_widget", [
                                ("float", "FloatSelect"),
                                ("int", "IntSelect")])
def test_SimpleData_input_number_open(mocker,
                                      qtbot,
                                      structure,
                                      ttype,
                                      input_widget):
...
@pytest.mark.parametrize("ttype, units, test_data, expected", [
                                ("int", None, 1, None),
                                ("bool", ["kg"], False, None),
                                ("int", ["kg"], 1, "kg")])
def test_SimpleData_output(mocker,
                           qtbot,
                           structure,
                           ttype,
                           units,
